<template>
  <div class="h-full">
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-5 h-full flex flex-col">
      ...
      <div v-else-if="!result?.is_eligible" class="p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">
      ...
      <div v-else-if="result" class="flex-1 flex flex-col justify-between">
        ...
        <div class="mt-3 pt-3 border-t border-gray-100">
          <p class="text-xs text-gray-500 leading-tight">* 주 15시간 이상 근무 시 발생합니다.</p>
        </div>
      </div>
    </div>
  </div>
